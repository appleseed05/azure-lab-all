...
runcmd:
  ...
  - |
    for i in $(seq 1 60); do
      if curl -fsSL -x "http://${proxy_ip}:${proxy_port}" https://apt.grafana.com/gpg.key \
           | gpg --dearmor -o /etc/apt/keyrings/grafana.gpg; then
        echo "grafana signing key fetched on attempt $i"
        break
      fi
      echo "grafana key fetch failed (attempt $i) - proxy not ready, retrying in 20s..."
      sleep 20
    done
  - |
  ...
  - apt_retry install -y loki grafana alloy
  ...
  # QUOTED ON PURPOSE. Unquoted, YAML reads "chown -R loki" as a KEY and
  # "/var/lib/loki" as its value, so this list item becomes a dict. cloud-init's
  # shellify() then raises TypeError and ABORTS THE WHOLE runcmd LIST - nothing
  # on this VM gets installed and the only clue is `cloud-init status` = error.
  ...
  - |
    # Wait for the stack to actually come up before asserting. A flat `sleep 8`
    # was not enough - Loki builds its index and Grafana preinstalls the loki
    # plugin through the proxy - so the checks fired too early and logged
    # ERRORs for a stack that was in fact healthy. Poll instead, up to ~2.5 min.
    for i in $(seq 1 30); do
      up=0
      curl -sf -o /dev/null http://127.0.0.1:3100/ready && up=$((up+1))
      curl -sf -o /dev/null http://127.0.0.1:3000/api/health && up=$((up+1))
      [ "$up" -eq 2 ] && break
      sleep 5
    done
