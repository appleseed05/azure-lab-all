...
resource "azurerm_linux_virtual_machine" "tf_azure_vm_rtr" {
  ...
  # Same keypair as every other VM, so the jumphost can reach the router without
  # a password. VyOS ingests the Azure-injected key through cloud-init and
  # regenerates authorized_keys from its own config - the file is generated, so
  # it must never be edited directly on the box. Password auth stays enabled
  # above, so this is additive: if the key does not take, the password still works.
  admin_ssh_key {
...
locals {
  ...
  # Sources allowed to reach the Internet DIRECTLY, bypassing Tinyproxy.
  # Everything else in the VNet is dropped (and logged) on the router's eth0.
  # Add entries here to exempt more hosts - e.g. the XC CEs
